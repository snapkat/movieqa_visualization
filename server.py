...
def w2v_movies(algo):
    if algo not in algos:
        return bottle.template("error",
                               message='Algorithm %s not found.' % algo)
...
def plot(algo, imdb_key, query_word=None):
    if imdb_key not in imdb_keys:
        return bottle.template("error",
                               message='Imdb key %s not found.' % imdb_key)
    if algo not in algos:
        return bottle.template("error",
                               message='Algorithm %s not found.' % algo)
...
def tfidf_plot(imdb_key, query_word):
    ...
    params = {'imdb_key': imdb_key, 'plot': origPlot, 'clean_words': cleanPlot,
              'weights': weights, 'query_word': None, 'rand': random.random(),
              'w2v': False, 'top_plot_words': top_plot_words}
    return bottle.template("plot_display_tfidf", **params)
...
def get_top_words(weights, clean_words, query_word=None, max_top=6):
    ...
    while len(top_words) < max_top and i >= 0:
        ...
        if currword not in top_words and currword != query_word:
...
def w2v_plot(imdb_key, query_word):
    ...
    params = {'imdb_key': imdb_key, 'plot': plot, 'clean_words': clean_words,
              'weights': weights, 'query_word': query_word,
              'rand': random.random(), 'w2v': True,
...
def display_movies(algo):
    ...
    return bottle.template("movie_select", imdb_keys=imdb_keys[:num_posters],
                           rows=rows, rand=random.random(), w2v=(algo == 'w2v')
                           )
